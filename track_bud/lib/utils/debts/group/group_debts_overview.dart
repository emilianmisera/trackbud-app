  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: groupProvider
          .getGroupDebtsOverview(groupId), // Fetch debts overview for the group
      builder: (context, snapshot) {
        ...
        List<Map<String, dynamic>> allPayments =
            snapshot.data ?? []; // Get all payments data
        String currentUserId =
            userProvider.currentUser?.userId ?? ''; // Get current user ID

        // Filter payments to get only the ones involving the current user
        List<Map<String, dynamic>> userPayments = allPayments
            .where((payment) =>
                payment['from'] == currentUserId ||
                payment['to'] == currentUserId)
            .toList();

        // Check if the user has any payments
        if (userPayments.isEmpty) {
          return const Text(
              'You have no outstanding debts or credits.'); // Inform user if no debts or credits
        }
        ...
        return CustomShadow(
          child: Container(
            width: MediaQuery.of(context)
                .size
                .width, // Set width to match the screen
            padding: const EdgeInsets.all(
                CustomPadding.smallSpace), // Padding around the container
            ...
            child: Column(
              ...
              children: [
                ...
                ...userPayments.map((payment) {
                  bool isDebtor = payment['from'] ==
                      currentUserId; // Check if current user is a debtor
                  String otherPersonId = isDebtor
                      ? payment['to']
                      : payment['from']; // Determine the other person's ID
                  String otherPerson = isDebtor
                      ? payment['toName']
                      : payment['fromName']; // Get the other person's name
                  double amount =
                      payment['amount'].toDouble(); // Get the amount
                  DebtsColorScheme colorScheme = isDebtor
                      ? DebtsColorScheme.red
                      : DebtsColorScheme
                          .green; // Set color scheme based on debt status

                  return FutureBuilder<UserModel?>(
                    future: firestoreService.getUser(
                        otherPersonId), // Fetch other person's user model
                    builder: (context, userSnapshot) {
                      ...
                      if (userSnapshot.hasData &&
                          userSnapshot.data!.profilePictureUrl.isNotEmpty) {
                        ...
                        leadingWidget = ClipRRect(
                          ...
                          child: Image.network(
                            userSnapshot.data!
                                .profilePictureUrl, // Load profile picture from network
                            width: 30,
                            height: 30,
                            fit: BoxFit.cover,
                          ),
                        ...
                        title: Text(otherPerson,
                            style: TextStyle(
                                color: defaultColorScheme
                                    .primary)), // Display other person's name
                        trailing: BalanceState(
                          colorScheme:
                              colorScheme, // Set color scheme for balance
                          amount:
                              '${amount.abs().toStringAsFixed(2)}€', // Display amount
                        ),
                      );
                    },
                  );
