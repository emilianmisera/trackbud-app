...
class GroupCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final defaultColorScheme = Theme.of(context)
        .colorScheme; // Get the default color scheme from the theme.
    final firestoreService =
        FirestoreService(); // Create an instance of FirestoreService to interact with Firestore.
      ...
      child: CustomShadow(
        child: Container(
          ...
          padding: const EdgeInsets.all(CustomPadding
              .defaultSpace), // Add default padding around the container.
          decoration: BoxDecoration(
            color: defaultColorScheme
                .surface, // Set the background color of the container.
            borderRadius: BorderRadius.circular(Constants
                .contentBorderRadius), // Round the corners of the container.
          ),
          child: Column(
            ...
            children: [
              ListTile(
                leading: ClipRRect(
                  borderRadius: BorderRadius.circular(
                      100.0), // Create a circular profile picture.
                  child: SizedBox(
                    width: 40,
                    height: 40,
                    child: group.profilePictureUrl.isNotEmpty
                        ? Image.network(group.profilePictureUrl,
                            fit: BoxFit
                                .cover) // Load group profile picture from URL.
                        : const Icon(Icons.group,
                            color: Colors
                                .grey), // Default icon if no profile picture is available.
                  ),
                ),
                title: Text(
                  group.name,
                  style: TextStyles.regularStyleMedium.copyWith(
                      color: defaultColorScheme
                          .primary), // Display group name with styling.
                ),
                subtitle: Text(
                  ...
                  style: TextStyles.hintStyleDefault.copyWith(
                    ...
                    color:
                        defaultColorScheme.secondary, // Style for the subtitle.
                  ),
                ),
                trailing: Text(
                  ...
                  style: TextStyles.regularStyleMedium.copyWith(
                      color: defaultColorScheme
                          .primary), // Style for the trailing text.
                ),
              ...
              Divider(
                  color: defaultColorScheme
                      .outline), // Add a divider below the ListTile.
              const Gap(CustomPadding
                  .mediumSpace), // Add medium space between elements.
              Row(
                mainAxisAlignment: MainAxisAlignment
                    .spaceBetween, // Space elements evenly within the row.
                children: [
                  Expanded(
                    child: SizedBox(
                      ...
                      child: Stack(
                        children: List.generate(group.members.length, (index) {
                          final reverseIndex = group.members.length -
                              1 -
                              index; // Calculate the reversed index for stacking members.
                          return FutureBuilder<UserModel?>(
                            future: firestoreService.getUser(group.members[
                                reverseIndex]), // Fetch user data for each member.
                            builder: (context, snapshot) {
                              if (snapshot.connectionState ==
                                  ConnectionState.waiting) {
                                return const SizedBox
                                    .shrink(); // Placeholder while user data is loading.
                              }
                              if (snapshot.hasError || snapshot.data == null) {
                                return const Icon(Icons.person,
                                    color: Colors
                                        .grey); // Display default icon on error or if user not found.
                              }
                              final member = snapshot
                                  .data!; // Get the member data from snapshot.
                              return Positioned(
                                left: index *
                                    30, // Position members with some overlap based on their index.
                                child: Container(
                                  decoration: BoxDecoration(
                                    border: Border.all(
                                      color: defaultColorScheme
                                          .surface, // Set border color around the profile picture.
                                      width: 1.0, // Set border width.
                                    ),
                                    borderRadius: BorderRadius.circular(
                                        100.0), // Make the picture circular.
                                  ),
                                  child: ClipRRect(
                                    borderRadius: BorderRadius.circular(
                                        100.0), // Round the corners of the profile picture.
                                    child: SizedBox(
                                      width: 40,
                                      height: 40,
                                      child: member.profilePictureUrl.isNotEmpty
                                          ? Image.network(
                                              ...
                                              fit: BoxFit
                                                  .cover) // Load member profile picture from URL.
                  ...
                  BalanceState(
                    colorScheme:
                        colorScheme, // Pass the determined color scheme for balance.
                    amount: userCredit == 0
