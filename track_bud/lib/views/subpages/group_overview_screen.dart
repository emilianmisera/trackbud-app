...
class _GroupOverviewScreenState extends State<GroupOverviewScreen> {
  final FirestoreService _firestoreService =
      FirestoreService(); // Service to interact with Firestore
  ...
  Future<void> _refreshGroupData() async {
    ...
    await groupProvider
        .refreshGroupData(widget.groupId); // Refresh group data in provider
    groupProvider.invalidateDebtsOverviewCache(
        widget.groupId); // Invalidate cache for debts overview
  }

  @override
  Widget build(BuildContext context) {
    final defaultColorScheme =
        Theme.of(context).colorScheme; // Get the current theme's color scheme
    final currentUserId =
        Provider.of<UserProvider>(context, listen: false).currentUser?.userId ??
            ''; // Get current user ID
    final groupProvider =
        Provider.of<GroupProvider>(context); // Access group provider
      ...
      body: FutureBuilder<GroupModel>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Center(
                child:
                    Text('No group data available')); // Show message if no data
          }

          final currentGroup = snapshot.data!; // Get the fetched group data

          return StreamBuilder<QuerySnapshot<GroupSplitModel>>(
            stream: _firestoreService.getGroupSplitsStream(
                widget.groupId), // Listen for group splits
            builder: (context, snapshot) {
              ...
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                List<GroupSplitModel> splits =
                    snapshot.data?.docs.map((doc) => doc.data()).toList() ?? [];

                // Calculate expenses and credits for the current user
                double currentUserExpenses =
                    groupProvider.getCurrentUserExpenses(widget.groupId);
                double totalGroupExpense =
                    groupProvider.getGroupExpense(widget.groupId);
                double currentUserCredit =
                    groupProvider.getUserCredit(widget.groupId);
                double expensesPerPerson = currentGroup.members.isNotEmpty
                    ? totalGroupExpense / currentGroup.members.length
                    : 0; // Calculate expenses per member
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        InfoTile(
                          title:
                              '${AppTexts.overall} (${expensesPerPerson.toStringAsFixed(2)}€/Person)',
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            InfoTile(
                              title: currentUserCredit >= 0
                                  ? AppTexts.credits
                                  : AppTexts.debts,
                              amount:
                                  currentUserCredit.abs().toStringAsFixed(2),
                          ...
                          builder: (context, groupProvider, child) {
                            return FutureBuilder<List<Map<String, dynamic>>>(
                              future: groupProvider
                                  .getGroupDebtsOverview(widget.groupId),
                              builder: (context, snapshot) {
                                // Handle different states of the future
                                if (snapshot.connectionState ==
                                  ...
                                  return const Text(
                                      'No debts to display'); // Show message if no debts
                                }

                                return DebtsOverview(
                                    groupId: widget
                                        .groupId); // Display debts overview
                              },
                            );
                          },
                        ),
                          ...
                          itemBuilder: (context, index) {
                            return Padding(
                              ...
                              child: GroupSplitTile(
                                split:
                                    splits[index], // Individual split details
                                currentUserId:
                                    currentUserId, // Pass current user ID for reference
                              ),
                            );
                          },
                        ),
        ...
        builder: (context, snapshot) {
          ...
          return Container(
            color: defaultColorScheme
                .onSurface, // Set background color for bottom sheet
            child: Container(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  List<String> memberNames = await Future.wait(currentGroup
                      .members
                      .map((memberId) => _firestoreService.getUser(memberId))
                      .map((futureUser) =>
                          futureUser.then((user) => user?.name ?? 'Unknown')));
                  ...
                  if (mounted) {
                    ...
                    showModalBottomSheet(
                      ...
                      builder: (context) => AddGroupSplit(
                        selectedGroup:
                            currentGroup, // Pass the current group to the modal
                        memberNames:
                            memberNames, // Pass member names to the modal
